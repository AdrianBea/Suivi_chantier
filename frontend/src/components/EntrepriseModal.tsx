      ...
      <Modal onClose={onClose} width={480} titleId="entreprise-modal-title">
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid var(--nm-border)", display: "flex", alignItems: "center", gap: 14, flexShrink: 0, background: "var(--nm-base-raised)" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div id="entreprise-modal-title" style={{ fontSize: 18, fontWeight: 700, color: "var(--nm-text-primary)" }}>{isNew ? "Nouvelle entreprise" : "Modifier l'entreprise"}</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 14 }}>
            {FIELDS.map(({ key, label }) => {
              ...
              return (
                <div key={key}>
                  <label htmlFor={`entreprise-${key}`} style={{ display: "block", fontSize: 10, color: "var(--nm-text-faint)", textTransform: "uppercase", letterSpacing: "0.1em", fontFamily: "monospace", marginBottom: 6 }}>{label}</label>
                  <input id={`entreprise-${key}`} value={value} onChange={(e) => setValue(e.target.value)} style={{ width: "100%", background: "var(--nm-base-sunken)", border: "1px solid var(--nm-border-strong)", borderRadius: 7, padding: "9px 12px", fontSize: 13, color: "var(--nm-text-secondary)", fontFamily: "inherit" }}/>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ padding: "16px 28px", borderTop: "1px solid var(--nm-border)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, background: "var(--nm-base-raised)" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleSave} disabled={saving || !nom} style={{ padding: "9px 20px", background: "var(--nm-success)", border: "none", borderRadius: 7, color: "var(--nm-text-on-accent)", fontSize: 13, fontWeight: 700, cursor: saving || !nom ? "not-allowed" : "pointer", opacity: saving || !nom ? 0.6 : 1, fontFamily: "inherit" }}>
      ...
      {confirmDelete && (
        <Modal onClose={() => setConfirmDelete(false)} width={380} titleId="entreprise-delete-title">
          <div style={{ padding: "24px 24px 20px" }}>
            <div id="entreprise-delete-title" style={{ fontSize: 15, fontWeight: 700, color: "var(--nm-text-primary)", marginBottom: 8 }}>Supprimer cette entreprise ?</div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              ...
              <button onClick={handleDelete} style={{ padding: "9px 18px", background: "var(--nm-danger)", border: "none", borderRadius: 7, color: "var(--nm-text-on-accent)", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Supprimer</button>
            </div>
          </div>
        </Modal>
      )}
