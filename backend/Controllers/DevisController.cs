...
[ApiController]
[Route("api/devis")]
public class DevisController(AppDbContext db, IExtractionService extractionService) : ControllerBase
{
    [HttpPost("import")]
    ...
    {
        var userId = User.GetUserId();
        var devis = await db.Devis.FirstOrDefaultAsync(d => d.Id == id && d.UserId == userId);
        if (devis == null) return NotFound();
    ...
    {
        var userId = User.GetUserId();
        if (!await db.Devis.AnyAsync(d => d.Id == id && d.UserId == userId))
            return NotFound();
